...
public class AgencyController implements Initializable {
	...
	public void addAgencyButtonListener() {
		...
		ClientController.getClient().invoke().createAgency(agency);
	...
	public void idSearchListener() {
		...
		ArrayList<Agency> ag = (ArrayList<Agency>) ClientController.getClient().invoke().getAgencyByID(id);
		if( ag.size() != 0) {
	...
	public void nameSearchListener() {
		...
		ArrayList<Agency> ag = (ArrayList<Agency>) ClientController.getClient().invoke().getAgencyByName(name);
		if( ag.size() != 0) {
			Iterator<Agency> it = ag.iterator();
			Agency agency;
			while(it.hasNext()) {
	...
	private ObservableList<Agency> getInitialTableData() {
		ArrayList<Agency> list = ClientController.getClient().invoke().getAgencies();
